...
app.config(function ($translateProvider) {
    $translateProvider.translations('fr', {
        ...
        PLANNING_INFO1:'un minimum de 2 personnes est exigé, sans quoi le cours sera malheureusement annulé.',
        CONTACT: 'Contacts',
        CONNEXION: 'CONNEXION',
        EQUIPE: 'L\'Equipe',
    ...
    Nous enseignons ce que nous aimons à flamma, et nous sommes présents pour vous accompagner dans votre propre aventure',
    ...
    CONTACT_TXT1: 'n\'hésitez pas à utiliser le formulaire ci-dessous pour nous laisser un message. Nous vous répondrons dans les plus brefs délais.',
        ...
        PROFIL_P6: 'Mes réservations',
        PROFIL_P6_2: 'Mes lignes de crédit',
        PROFIL_P7: 'Modifier votre compte',
        PROFIL_P8: 'Accéder au pratique',
        PROFIL_P9: 'Accéder au planning',
        ...
        PROFIL_LC1: 'Vos lignes de crédit',
        PROFIL_LC2: 'Vous n\'avez pas encore de ligne de crédit à votre actif!',
        ...
        MON_PANNIER_Q:'Quantité',
        MON_PANNIER_CONTINUER:'CONTINUER MES ACHATS',
        MON_PANNIER_RECAP:'RÉCAPITULATIF',
        MON_PANNIER_TTC:'TTC',
        ...
        MON_PANNIER_2_ML_Q:'Comment voulez vous que votre commande soit livrée ?',
...
app.controller('afterLoginCtl', function (Init,panierLocalStorage, panier, userLogged, $location, $scope, $cookies, $filter, socialLoginService, $log, $q, $translate, $http, $window) {
    ...
    $scope.addElement = function (type, idItem, stepp, items)
    {
        ...
        if ($scope.userConnected) {
            ...
            {
                iziToast.error({
                    message: '<span class="h4">Un problème est survenu <br> Veuillez contactez l\'administrateur</span>',
                    position: 'topRight'
                });
                console.log('Erreur serveur ici = ' + msg);
            });
    ...
    $scope.addToPanierOffLine = function (type, produit) {
        ...
        try {
            ...
            {
                iziToast.error({
                    message: "Mettre une quantité valide",
                    displayMode: 'once',
                    position: 'topRight'
                });
                return false;
            ...
            else if (parseInt(quantite) == 0)
            {
                ...
                $scope.showToast('Votre panier à été mis à jour avec succès', true);
            }
            else {
                var total;
